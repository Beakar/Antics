...
class AIPlayer(Player):
    ...
    def __init__(self, inputPlayerId):
        super(AIPlayer, self).__init__(inputPlayerId, "Food Gatherer: The Return")
    ...
    def depthLimitedSearch(self, state, playerID, currDepth, currNode):
        ...
        for move in moves:
            ...
            if (node.eval > max(0.80, currEval + (currEval / 25)) and nextState.whoseTurn == self.playerId ):
                node.eval = self.depthLimitedSearch(nextState, nextState.whoseTurn, currDepth + 1, node)
            #elif (node.eval > min(0.20, currEval - (currEval / 25)) and nextState.whoseTurn != self.playerId ):
            elif nextState.whoseTurn != self.playerId:
                node.eval = self.depthLimitedSearch(nextState, nextState.whoseTurn, currDepth + 1, node)
        ...
        if currDepth == 0:
            return self.bestMoveInNodeList(nodeList, playerID)
        else:
            return self.evalNodes(nodeList, playerID)
    ...
    ##
    def bestMoveInNodeList(self, nodeList, playerID):
        bestMove = None
        maximum = 0.0
        minimum = 1.0
        ...
        for node in nodeList:
            if self.playerId is playerID and node.eval > maximum:
                bestMove = node.move
                maximum = node.eval
            elif self.playerId is not playerID and node.eval < minimum:
                bestMove = node.move
                minimum = node.eval
    ...
    ##
    def evalNodes(self, nodeList, playerID):
        bestMove = None
        maximum = 0.0
        minimum = 1.0
...
class AITests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.state = GameState(board, [self.p1Inventory, self.p2Inventory], PLAY_PHASE, PLAYER_ONE)
    ...
    def testAnthillLoc(self):
        self.assertTrue(self.state.board[1][8].constr is not None and self.state.board[1][8].constr.type == ANTHILL)
        self.assertTrue(self.state.board[1][1].constr is not None and self.state.board[1][8].constr.type == ANTHILL)
    ...
    def testSameAntType(self):
        self.assertEqual(self.state.board[0][3].ant.type, self.state.board[6][2].ant.type)
        self.assertEqual(self.state.board[4][6].ant.type, self.state.board[6][7].ant.type)
    ...
    def testStayStillMove(self):
        ...
        self.assertEqual(newState.board[antCoords[0]][antCoords[1]].ant.coords, antCoords)
        self.assertTrue(newState.board[antCoords[0]][antCoords[1]].ant.hasMoved)
        # Make sure the new state has the value we expect
        self.assertEqual(val, 0.877)
    ...
    def testFoodPickUpAndDrop(self):
        ...
        self.assertTrue(newState.board[6][2].ant.carrying is False)
        self.assertTrue(newState.board[0][3].constr.type == FOOD)
        self.assertTrue(newState.board[0][3].ant.carrying)
        # Make sure the new state has the value we expect
        self.assertEqual(val, 0.841)
    ...
    ##
    def tearDown(self):
        print "Test passed for Food Gatherer: The Return"
